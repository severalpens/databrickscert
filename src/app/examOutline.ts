    ...
    topics: [
      {
        ...
        answer:
          "A data warehouse is a data repository that can be used by data analyst or data scientist. A data lake is a generic location where structured and unstructured data can be stored. A data lakehouse is a combination of the two, where structured data is stored in a data warehouse and unstructured data is stored in a data lake. The data lakehouse allows for the flexibility of a data lake while providing the structure and performance of a data warehouse.",
      },
      {
        id: 1.2,
        topic:
        ...
        answer:
          "Data quality in the data lakehouse is improved over the data lake by providing a structured environment for data storage and processing. This includes features such as schema enforcement, ACID transactions, and data governance capabilities that ensure data integrity and consistency.",
      },
      {
        id: 1.3,
        topic:
        ...
        answer:
          "Bronze tables are typically used for raw, unprocessed data, serving as the initial landing zone for data ingestion. Silver tables are used for cleaned and transformed data, often serving as a source for more complex analytics. Gold tables are used for high-quality, aggregated data that is ready for reporting and business intelligence. Workloads that require raw data will use bronze tables, while workloads that require processed data will use silver or gold tables.",
      },
      {
        id: 1.4,
        topic:
      ...
      },
      {
        ...
        answer:
          "Databricks cluster software is versioned by the Databricks Runtime by assigning a specific version number to each release. This versioning allows users to select the appropriate runtime version for their clusters, ensuring compatibility with their workloads and access to the latest features and improvements.",
      },
      {
        id: 1.7,
        topic:
        ...
        answer:
          "Clusters can be filtered by user access permissions, allowing users to view only those clusters they have permission to access. This can be done through the Databricks UI or API, where users can apply filters based on cluster status, owner, or other attributes.",
      },
      {
        id: 1.8,
        topic:
      ...
      },
      {
        ...
        answer:
          "A scenario in which restarting the cluster will be useful is when there are performance issues or memory leaks in the cluster. Restarting the cluster can help clear any cached data, free up resources, and reset the environment to a clean state, improving overall performance.",
      },
      {
        id: 1.1,
        topic:
      ...
      },
      {
        ...
        answer:
          "Databricks Notebooks version control functionality is limited compared to Repos in that it does not support advanced Git features such as branching, merging, and pull requests. Notebooks version control primarily tracks changes to the notebook content itself, while Repos provides a full Git experience with all the associated capabilities for collaborative development and version management.",
      },
    ],
  },
  {
    ...
    topics: [
      ...
      },
      {
        ...
        answer:
          "The prefix included after the FROM keyword in a SQL query indicates the data source type, such as 'table', 'view', 'file', or 'directory'. For example, `FROM table_name` indicates a table, while `FROM '/path/to/file.csv'` indicates a file.",
      },
      {
        id: 2.3,
        topic:
      ...
      },
      {
        ...
        answer:
          "You can identify tables from external sources by checking their format and metadata. If a table is not stored in Delta Lake format, it will not have the Delta Lake transaction log and will not support ACID transactions, schema enforcement, or time travel features.",
      },
      {
        id: 2.5,
        topic:
      ...
      },
      {
        ...
        answer:
          "The `PIVOT` clause is used to convert data from a long format (where each row represents a single observation) to a wide format (where each unique value in a specified column becomes a separate column). For example:\n\n```sql\nSELECT * FROM my_table\nPIVOT (\n  SUM(value_column)\n  FOR category_column IN ('category1', 'category2', 'category3')\n);\n```\nThis will create new columns for each category with the corresponding summed values.",
      },
      {
        id: 2.23,
        topic: "Define a SQL UDF.",
        answer:
          "A SQL UDF (User-Defined Function) is a custom function that allows users to define their own logic and operations in SQL queries. It can be used to encapsulate complex calculations or transformations that are not available as built-in functions. UDFs can be written in SQL or in programming languages like Python or Scala, depending on the environment.",
      },
      {
        id: 2.24,
        topic: "Identify the location of a function.",
        answer:
          "The location of a function can be identified by its schema and name. In Databricks, functions can be stored in the default schema or in a user-defined schema. You can use the `SHOW FUNCTIONS` command to list all functions available in a specific schema, or you can query the `information_schema` to find details about a specific function.",
      },
      {
        id: 2.25,
        topic: "Describe the security model for sharing SQL UDFs.",
        answer:
          "The security model for sharing SQL UDFs in Databricks involves defining access controls and permissions for the UDFs. Users can grant or revoke permissions to specific users or groups, allowing them to execute the UDFs. Additionally, UDFs can be shared across different workspaces or clusters, ensuring that only authorized users can access and use them.",
      },
      {
        id: 2.26,
        topic: "Use CASE/WHEN in SQL code.",
        answer:
          "To use `CASE/WHEN` in SQL code, you can create conditional logic that evaluates expressions and returns different values based on the conditions specified. For example:\n\n```sql\nSELECT column_name,\n       CASE\n         WHEN condition1 THEN result1\n         WHEN condition2 THEN result2\n         ELSE default_result\n       END AS new_column\nFROM my_table;\n```\nThis will create a new column with values based on the conditions evaluated for each row.",
      },
      {
        id: 2.27,
